...
def convert_to_df(data):
    ...
    dataframe = {
        ...
        "text": [[row['text'] for row in row.get('includes')['tweets']][0] if row["text"].encode("utf-8").startswith("RT @") and y:=row.get('includes') and y.get('includes')['tweets'] else row["text"].encode("utf-8") for row in data],
