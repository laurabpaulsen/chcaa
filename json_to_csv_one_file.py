...
def convert_to_df(data):
    ...
    dataframe = {
        ...
        "mentionee": [get_mentionee1([row['username'] for row in  [row['entities']['mentions'] for row in row.get('includes')['tweets']][0]]) if row["text"].startswith("RT @") else get_mentionee(row['entities'].get('mentions')) for row in data],
